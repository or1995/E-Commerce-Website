...
import classes from './FullSearch.module.css';
...
class FullSearch extends Component {
    ...
    fetchData = () => {
        axios.get(`/product/api/products/?ordertype=${this.state.order}&key=${this.props.match.params.key}&datanumsearch=${this.state.currentdatafetch}`)
        .then(res => {
            this.setState({
    ...
    componentDidMount() {
        axios.get(`/product/api/products/?ordertype=${this.state.order}&key=${this.props.match.params.key}&datanumsearch=0`)
            .then(res => {
                this.setState({
                    hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                    products: this.state.products.concat(res.data)
    ...
    dateorder = () => {
        ...
        if(this.state.order === '-pub_date') {
            ...
            axios.get(`/product/api/products/?ordertype=pub_date&key=${this.props.match.params.key}&datanumsearch=0`)
            .then(res => {
                this.setState({
                    hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                    products: res.data
    ...
    priceorder = () => {
        ...
        if(this.state.order === '-price') {
            ...
            axios.get(`/product/api/products/?ordertype=price&key=${this.props.match.params.key}&datanumsearch=0`)
            .then(res => {
                this.setState({
                    hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                    products: res.data
    ...
    nameorder = () => {
        ...
        if(this.state.order === '-name') {
            ...
            axios.get(`/product/api/products/?ordertype=name&key=${this.props.match.params.key}&datanumsearch=0`)
            .then(res => {
                this.setState({
                    hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                    products: res.data
    ...
    viewsorder = () => {
        ...
        if(this.state.order === '-views') {
            ...
            axios.get(`/product/api/products/?ordertype=views&key=${this.props.match.params.key}&datanumsearch=0`)
            .then(res => {
                this.setState({
                    hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                    products: res.data
    ...
    render() {
        return (
            <div className={classes.productsfullsearch}>
                <div className={classes.productstitle}>
                    <h2>Search results for '{this.props.match.params.key}'</h2>
