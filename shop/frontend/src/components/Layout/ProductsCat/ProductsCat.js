...
class ProductsCat extends Component {
    ...
    fetchData = () => {
        axios.get(`/product/api/products/?ordertype=${this.state.order}&category=${this.props.category}&datanumcat=${this.state.currentdatafetch}`)
        .then(res => {
            this.setState({
    ...
    componentDidMount() {
        axios.get(`/product/api/products/?ordertype=${this.state.order}&category=${this.props.category}&datanumcat=0`)
        .then(res => {
            this.setState({
                hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                products: this.state.products.concat(res.data),
    ...
    dateorder = () => {
        ...
        if(this.state.order === '-pub_date') {
            ...
            axios.get(`/product/api/products/?ordertype=pub_date&category=${this.props.category}&datanumcat=0`)
            .then(res => {
                this.setState({
                    hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                    products: this.state.products.concat(res.data),
    ...
    priceorder = () => {
        ...
        if(this.state.order === '-price') {
            ...
            axios.get(`/product/api/products/?ordertype=price&category=${this.props.category}&datanumcat=0`)
            .then(res => {
            this.setState({
                hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                products: this.state.products.concat(res.data),
    ...
    nameorder = () => {
        ...
        if(this.state.order === '-name') {
            ...
            axios.get(`/product/api/products/?ordertype=name&category=${this.props.category}&datanumcat=0`)
            .then(res => {
            this.setState({
                hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                products: this.state.products.concat(res.data),
    ...
    viewsorder = () => {
        ...
        if(this.state.order === '-views') {
            ...
            axios.get(`/product/api/products/?ordertype=views&category=${this.props.category}&datanumcat=0`)
            .then(res => {
            this.setState({
                hasMore: this.state.products.length === this.state.products.concat(res.data).length ? false : true,
                products: this.state.products.concat(res.data),
    ...
    render() {
        return (
            <div className={classes.productscat}>
